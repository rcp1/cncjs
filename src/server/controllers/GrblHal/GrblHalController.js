...
class GrblHalController {
    ...
    constructor(engine, options) {
        ...
        this.feeder.on('data', (line = '', context = {}) => {
            ...
            // Acknowledge tool change
            if (_.includes(line, 'M6')) {
                this.event.trigger('acknowledge_tool_change');
            }
            log.silly(`> ${line}`);
        });
        ...
        this.runner.on('error', (res) => {
            ...
            const error = _.find(GRBLHAL_ERRORS, { code: code });
        ...
        this.queryTimer = setInterval(() => {
            ...
            if (this.settings !== this.runner.settings) {
                ...
                this.emit('GrblHal:settings', this.settings); // Backward compatibility
            }
            ...
            if (this.state !== this.runner.state) {
                ...
                this.emit('GrblHal:state', this.state); // Backward compatibility
            }
    ...
    populateContext(context) {
        ...
        return Object.assign(context || {}, {
            ...
            modal: {
                ...
                canned_cycles_return: modal.canned_cycles_return,
    ...
    addConnection(socket) {
        ...
        if (!_.isEmpty(this.settings)) {
            ...
            socket.emit('GrblHal:settings', this.settings); // Backward compatibility
        }
        if (!_.isEmpty(this.state)) {
            // controller state
            socket.emit('controller:state', GRBLHAL, this.state);
            socket.emit('GrblHal:state', this.state); // Backward compatibility
        }
        if (this.feeder) {
            // feeder status
